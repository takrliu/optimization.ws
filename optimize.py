...
def effective_damage(effective_power, s, c, crit_chance, crit_severity, st, ct, t, n):
	damage = n*(effective_power*s + c + t*(effective_power*st +ct))
	expected_average_damage = damage*(1 + crit_chance*(crit_severity-1))
...
class Skill:
	def __init__(self):
		self.skill ={}
		self.info = {}
	def add_skill(self, name, local_max, unscaled_power, effective_power, critical_chance, effective_damage, t, w, n):
		self.skill[name] = {}
		self.skill[name]['max'] = local_max
		self.skill[name]['ed'] = effective_damage
...
class Optimize:
	...
	def optimize_skill(self, skill):
		...
		a_effective_damage = [effective_damage(ep, s, c, cc, self.chs, st, ct, t, n) for ep, cc in zip(a_effective_power, a_critical_chance)]
		...
		local_maximum = [y_max, x_max, a_critical_chance[ymax_idx]*100]

		# store data
		self.s.add_skill(self.skill_names[skill_index], 
				local_maximum,
				a_unscaled_power,
				a_effective_power,
				a_critical_chance,
		...
		print 
		return [x_max, a_critical_chance[ymax_idx]]
	...
	def optimize_all(self):
		...
		for i, s in enumerate(self.s.skill.keys()):
			print s, self.s.skill[s]['tier'], self.s.skill[s]['weight'], self.s.skill[s]['nhit']
			...
			# for i in self.s.skill[s]['ed']:
			# 	print i, i*self.s.skill[s]['weight']
		...
		global_maximum = [y_max, x_max, self.s.info['cc'][ymax_idx]*100]

		print 

		# print self.skill_names[skill_index].upper() + ' Tier: ' + str(int(t))
		global_max_string = '%0.2f\n%0.2f AP\n%0.2f%%' % (y_max, x_max, self.s.info['cc'][ymax_idx]*100)
